...
import { getProductsInCartAPI, updateCheckedStatusAPI } from "../../apis/postAPIs";
...
function Cart() {
  ...
  const handleQuantityChange = async (id, newQuantity) => {
    try {
      const response = await updateQuantityAPI(userData.id, id, newQuantity);
      if (response) {
  ...
  const handleRemove = async (id) => {
    try {
      const response = await removeProductAPI(userData.id, id);
  ...
  const handleCheck = async (id) => {
    try {
      const response = await updateCheckedStatusAPI(userData.id, id); 
